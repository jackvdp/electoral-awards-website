...
argument-hint: [topic or brief, optional target size]
...
Use `AskUserQuestion` for these. Also ask for anything needed by the eventual email but not by the list itself (dates, price, booking link) and flag that it is not blocking, then get on with the research.
...
- `mcp__hunter__Get-Usage` - credits and verifications remaining, and the reset date. Free.
- `mcp__hunter__Find-People` with a `domains` array of up to 100 candidate domains - returns how many personal and generic emails Hunter holds per domain. Free.
- `mcp__hunter__Find-Companies` - free, useful when you need to discover orgs rather than confirm them.
...
The harness persists any oversized tool result to a file and shows you only a short preview. Exploit that deliberately:

- Call Domain-Search with **`limit: 100`** and **no narrow filters**. Bigger payloads spill to disk; small filtered ones land in context.
- Roughly 50KB is the spill threshold. Organisations with more than about 40 indexed contacts reliably spill.
- Persisted files land in the session tool-results directory. Find them with:
  ```bash
  ls ~/.claude/projects/<project-slug>/<session-id>/tool-results/
  ```
- If a mid-sized organisation lands in context anyway, re-request it with `limit: 100` and the filters removed so it spills. Costs a few credits, saves far more context.
- Batch several domains per message so they run concurrently.
...
2. Scrape the staff, leadership, board or team page with Exa `web_fetch_exa`, falling back to `mcp__firecrawl__firecrawl_scrape` for JS-heavy sites.
3. Generate candidate addresses by applying the pattern to each scraped name.
4. **Verify them** with `mcp__hunter__Email-Verifier` before they go anywhere near a send. Generated-and-unverified is the one combination to avoid.
