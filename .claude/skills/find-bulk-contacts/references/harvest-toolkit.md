...
Oversized tool results are written to the session's tool-results directory rather than into context.

```bash
TR=~/.claude/projects/<project-slug>/<session-id>/tool-results
ls "$TR"
...
Both naming styles appear (`toolu_*.txt` and `mcp-hunter-Domain-Search-*.txt`); glob for `*.txt` and let `jq` skip anything that is not a Domain-Search payload.
...
Take `pattern` from the org's Domain-Search response. Then verify every generated address with `mcp__hunter__Email-Verifier` before it goes into the CSV, and mark it `pattern` unless the verifier returns `valid`.
