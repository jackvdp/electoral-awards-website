...
- **Committee membership pages** at `https://committees.parliament.uk/committee/<id>/<slug>/membership/`. The client’s page-fetch tool returns 403 on these — **use Exa's `web_fetch_exa`** (or Firecrawl) instead. The page lists current members with party, constituency, and chair role.
