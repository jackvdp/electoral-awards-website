...
.agents/skills/email-inbox/fetch.sh

# Fetch a specific number
.agents/skills/email-inbox/fetch.sh --max 10

# Search by subject or sender
.agents/skills/email-inbox/fetch.sh --search "COMELEC"

# Paginate — skip the first 20, fetch the next 30
.agents/skills/email-inbox/fetch.sh --offset 20 --max 30
```
...
.agents/skills/email-inbox/mailboxes.sh --list
.agents/skills/email-inbox/mailboxes.sh --list --filter "Awards 26"

# Browse a mailbox (most recent first, headers only — fast)
.agents/skills/email-inbox/mailboxes.sh --mailbox "Awards 26 Sponsors" --max 30

# Search a mailbox by subject or sender
.agents/skills/email-inbox/mailboxes.sh --mailbox "BSVA" --search "workshop"

# Read matching bodies
.agents/skills/email-inbox/mailboxes.sh --mailbox "BSVA" --search "workshop" --full

# Search sent mail (works fast even though Sent Items holds ~25k messages)
.agents/skills/email-inbox/mailboxes.sh --mailbox "Sent Items" --search "Symposium Review"
```
...
All scripts are in `.agents/skills/email-inbox/`.
...
```bash
.agents/skills/email-inbox/fetch.sh --search "sender@example.com" --max 1
```
...
.agents/skills/email-inbox/reply.sh \
...
```bash
# Basic new compose
.agents/skills/email-inbox/compose.sh \
