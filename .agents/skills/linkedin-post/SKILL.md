...
| Topic | Source files |
|-------|-------------|
| Events / Awards | `src/data/schedule.ts`, `src/data/award-categories.ts`, AGENTS.md |
...
```bash
POST_TEXT='The post text here' node .Codex/skills/linkedin-post/publish.mjs
```

To include an article link attachment:

```bash
POST_TEXT='The post text here' POST_URL='https://electoralnetwork.org/articles/...' POST_TITLE='Article Title' node .Codex/skills/linkedin-post/publish.mjs
```
...
```bash
node .Codex/skills/linkedin-post/oauth-setup.mjs
```
