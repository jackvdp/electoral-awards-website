...
| Topic | Source files |
|-------|-------------|
| Events / Awards | `web/src/data/schedule.ts`, `web/src/data/award-categories.ts`, AGENTS.md |
...
```bash
POST_TEXT='The post text here' node .agents/skills/linkedin-post/publish.mjs
```

To include an article link attachment:

```bash
POST_TEXT='The post text here' POST_URL='https://electoralnetwork.org/articles/...' POST_TITLE='Article Title' node .agents/skills/linkedin-post/publish.mjs
```
...
```bash
node .agents/skills/linkedin-post/oauth-setup.mjs
```
