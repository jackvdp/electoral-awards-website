...
.Codex/skills/apple-mail-inbox/fetch.sh

# Fetch a specific number
.Codex/skills/apple-mail-inbox/fetch.sh --max 10

# Search by subject or sender
.Codex/skills/apple-mail-inbox/fetch.sh --search "COMELEC"

# Paginate — skip the first 20, fetch the next 30
.Codex/skills/apple-mail-inbox/fetch.sh --offset 20 --max 30
```
...
All scripts are in `.Codex/skills/apple-mail-inbox/`.
...
```bash
.Codex/skills/apple-mail-inbox/fetch.sh --search "sender@example.com" --max 1
```
...
.Codex/skills/apple-mail-inbox/reply.sh \
