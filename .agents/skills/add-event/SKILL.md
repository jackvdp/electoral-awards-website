...
Events on this site are stored in MongoDB (not static files). To add a new event, you run the generic seed script at `.Codex/skills/add-event/seed-event.mjs` with the event data passed as a JSON environment variable.
...
```bash
EVENT_JSON='{ ... }' node .Codex/skills/add-event/seed-event.mjs
```
...
EVENT_JSON='{
  ...
  "speakers": []
}' node .Codex/skills/add-event/seed-event.mjs
```
